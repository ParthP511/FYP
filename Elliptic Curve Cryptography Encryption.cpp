...
ellipticCurveFq& E_Fq)
{
     ...
     if (R.size())
     {
convertElementToInt(Integer& R, Integer& mod,
...
bool       SignatureVerification(SignatureECDSA&
message,ecPoint& P,ecPoint& Q,ellipticCurveFq& E_Fq,Integer& n)
{
...
E_Fq.scalarMultiply(S.R,P,k,n); convertElementToInt(_x,S.R.x,n,E_Fq);//x co-ordinate of
point R of Signature S
     invin(k,n);
...
ecPoint V1,V2;
Integer _x;
convertElementToInt(_x,S.R.x,n,E_Fq);//x co-ordinate of
point R of Signature S
...
SignatureVerification(SignatureELGAMAL&
S,Integer&
